...
// Process emissions data for stacked bar chart
const processEmissionsData = () => {
  const years = Array.from(new Set(emissionsByRegion.map((item) => item.year)));
  const categories = Array.from(
    new Set(emissionsByRegion.map((item) => item.category)),
  );
  ...
  return years.map((year) => {
    ...
    categories.forEach((category) => {
      const dataPoint = emissionsByRegion.find(
        (item) => item.year === year && item.category === category,
      );
      if (dataPoint) {
        yearData[category as string] = dataPoint.value;
      }
    });
...
const processedEmissionsData = processEmissionsData();
    ...
    return (
      <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-md dark:border-slate-700 dark:bg-slate-800">
        ...
        {payload.map((entry, index) => (
          <div key={`tooltip-${index}`} className="mt-1 flex items-center">
            <div
            ...
            <span className="text-slate-600 dark:text-slate-300">
              ...
              {entry.name === "Temperature Anomaly" ? "°C" : "Gt"}
            </span>
          </div>
        ))}
      </div>
              ...
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    {/* violet-500 */}
                    <Bar dataKey="Asia" stackId="a" fill="#f97316" />{" "}
                    {/* orange-500 */}
                  </BarChart>
                </ResponsiveContainer>
              </div>
