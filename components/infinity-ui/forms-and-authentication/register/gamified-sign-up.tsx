...
import { ArrowRight, Check, GamepadIcon as GameController, Trophy, User, Mail, Lock } from "lucide-react"

const GamifiedSignUp = () => {
  ...
  useEffect(() => {
    let newScore = 0
    if (username) newScore += 25
    if (email) newScore += 25
    if (password) newScore += 25
    if (confirmPassword) newScore += 25
    setScore(newScore)
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col items-center justify-center overflow-hidden bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-900 p-4">
      ...
      >
        ...
        >
          <div className="p-8">
            <h1 className="mb-2 text-center text-2xl font-bold text-white">Create Your Character</h1>
            <p className="mb-6 text-center text-purple-300">Join the adventure and start your gaming journey</p>
                    ...
                    <div className="space-y-2">
                      ...
                      {email && (
                        <motion.p
                          ...
                          className="mt-1 text-xs text-green-400"
                        >
                          +25 XP! Communication channel established!
                        </motion.p>
                      )}
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      {password && (
                        <motion.p
                          ...
                          className="mt-1 text-xs text-green-400"
                        >
                          +25 XP! Security enchantment added!
                        </motion.p>
                      )}
                    </div>
          ...
          <div className="border-t border-white/10 bg-gray-800/50 p-6">
            <p className="text-center text-sm text-purple-300">
              ...
              <Link href="#" className="font-medium text-pink-400 transition-colors hover:text-pink-300">
                Sign in to play
              </Link>
            </p>
          </div>
        ...
        <div className="mt-8 rounded-xl border border-white/10 bg-gray-900/50 p-4 backdrop-blur-lg">
          ...
          <div className="grid grid-cols-4 gap-2">
            <motion.div
              className={`rounded-lg p-2 ${username ? "border border-purple-500/30 bg-purple-900/50" : "border border-gray-700/30 bg-gray-800/50"}`}
              whileHover={{ scale: 1.05 }}
            >
              <div className="text-center">
                <User className={`mx-auto mb-1 h-5 w-5 ${username ? "text-purple-400" : "text-gray-500"}`} />
                <span className={`text-xs ${username ? "text-purple-300" : "text-gray-500"}`}>Identity</span>
              </div>
            </motion.div>
            <motion.div
              className={`rounded-lg p-2 ${email ? "border border-blue-500/30 bg-blue-900/50" : "border border-gray-700/30 bg-gray-800/50"}`}
              whileHover={{ scale: 1.05 }}
            >
              <div className="text-center">
                <Mail className={`mx-auto mb-1 h-5 w-5 ${email ? "text-blue-400" : "text-gray-500"}`} />
                <span className={`text-xs ${email ? "text-blue-300" : "text-gray-500"}`}>Contact</span>
              </div>
            </motion.div>
            <motion.div
            ...
            >
              <div className="text-center">
                <Lock className={`mx-auto mb-1 h-5 w-5 ${password ? "text-pink-400" : "text-gray-500"}`} />
                <span className={`text-xs ${password ? "text-pink-300" : "text-gray-500"}`}>Security</span>
              </div>
            </motion.div>
            <motion.div
            ...
            >
              <div className="text-center">
                ...
                />
                <span className={`text-xs ${score === 100 ? "text-yellow-300" : "text-gray-500"}`}>Gamer</span>
              </div>
            </motion.div>
          </div>
        </div>
